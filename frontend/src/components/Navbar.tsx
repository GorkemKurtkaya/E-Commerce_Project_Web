...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      if (AuthCheck && userData) {
        const [authResult, user] = await Promise.all([AuthCheck(), userData()]);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (searchQuery) {
        try {
          const results = await searchProducts(searchQuery); // Await the results
          setSearchResults(results || []); // No TypeScript error now
        } catch (error) {
  ...
  useEffect(() => {
    // Clear search query and close dropdowns on page change
    setSearchQuery("");
    setIsDropdownOpen(false);
  }, [location.pathname]);
  ...
  const handleLogOut = async () => {
    try {
      const isLoggedOut = await fetchUserLogOut();
      if (!isLoggedOut) {
        navigate("/login");
